...
const totalProfit = filteredData.reduce((sum, item) => sum + (item.totalFee / DECIMALS * item.poolPrice) - (item.totalGasETH * item.ethPrice), 0).toFixed(2);

// Transform the data for table

const transformedData = filteredData.map(item => {
    const payoutAmount = (item.totalPayout / DECIMALS).toFixed(2);
    const feesAmount = (item.totalFee / DECIMALS).toFixed(2);
    ...
    const profit = "$"+((feesAmount * item.poolPrice) - (item.totalGasETH * item.ethPrice)).toFixed(2);
...
const totalWeightedFees = filteredData.reduce((sum, item) => sum + (item.totalFee / DECIMALS * item.poolPrice), 0);
const averageFeePrice = (totalWeightedFees / totalFees).toFixed(4);

// Display totals
console.log("");
console.log(`Total Payouts: ${totalPayouts.toFixed(2)} ${PRIZE}`);
console.log(`Total Fees: ${totalFees.toFixed(2)} (${totalFeesPercentage}) ${PRIZE}`);
