...
//const {estimateGas} = require("@ethers-ext/utils-optimism")
const {web3GasEstimate} = require("./utilities/web3")
const {AlchemyGas,AlchemyTransactionReceipt } = require("./utilities/alchemy")
const {GetPricesForToken} = require("./utilities/1inch")
const {BuildTxForSwap} = require("./utilities/1inchSwap.js")


// liquidation pair whitelist and blacklist
//const ONLYLIQUIDATE=['0x22C33b869Afda26514B8d18132e1548Da200a592']
const ONLYLIQUIDATE = []
const DONTLIQUIDATE=[]
...
const percentProfitThreshold = .02
const minimumGasCost = .03
...
/*
function scaleL2GasPrice(mainnetGasPriceGwei) {
 mainnetGasPriceGwei  = parseInt(mainnetGasPriceGwei)
 const mainnetGasPriceScalingFactor = 9; // Set the mainnet gas price scaling factor
  const l2GasPriceScalingFactor = 0.4 / mainnetGasPriceScalingFactor; // Calculate L2 gas price scaling factor

  // Calculate the scaled L2 gas price
  const scaledL2GasPriceGwei = (mainnetGasPriceGwei / mainnetGasPriceScalingFactor) * l2GasPriceScalingFactor;

  return parseInt(scaledL2GasPriceGwei);
}*/


// old gas junk
/*
const scaleL2GasPrice = async () => {
  const mainnetGasPrice = await PROVIDERS["MAINNET"].getGasPrice()
  console.log("mainnnet gas ",parseInt(parseInt(mainnetGasPrice)/1e9))
let addMainnetGas = 0
if(mainnetGasPrice / 1e9 > 40){console.log("mainnet gas over 40"); addMainnetGas += ((mainnetGasPrice/1e9)*.003)*1e9}

  // const scaledGas = parseInt(mainnetGasPrice * 0.01)
  // console.log("scaled gas",scaledGas)
  const alchemyGas = await AlchemyGas()
  console.log("alchemy gas",alchemyGas.toString())
  const totalGas = parseInt(alchemyGas) + addMainnetGas
  return totalGas

  //return 900000000 // manual override to price gas
  if( scaledGas < 31000000){ return 3100000 }else{ return scaledGas}  // l2 gas floor hardcoded to .042 gwei
}
...
if(!isAwarding){
  ...
  let walletPrizeTokenBalance = await CONTRACTS.POOL[CONFIG.CHAINNAME].balanceOf(
    CONFIG.WALLET
  );
...
if (ONLYLIQUIDATE.length > 0) {
    pairs = pairs.filter(pair => ONLYLIQUIDATE.map(addr => addr.toLowerCase()).includes(pair.LIQUIDATIONPAIR.toLowerCase()));
} else if (DONTLIQUIDATE.length > 0) {
    pairs = pairs.filter(pair => !DONTLIQUIDATE.map(addr => addr.toLowerCase()).includes(pair.LIQUIDATIONPAIR.toLowerCase()));
}

  //console.log(pairs)
  geckos = pairs.map((pair) => pair.GECKO);
  console.log(geckos);
  const geckoPrices = await GeckoIDPrices(geckos);
...
prizeTokenPrice = await GetPricesForToken(ADDRESS[CONFIG.CHAINNAME].PRIZETOKEN.ADDRESS)
await delay(900);  // Delays for 1.1 seconds

}
  const ethPrice = ethAndPoolPrices[1];

if(prizeTokenPrice===null || prizeTokenPrice === undefined || prizeTokenPrice===0) {console.log("cannot get prize token price");return}
  console.log("prize token $", prizeTokenPrice, " eth $", ethPrice);
  console.log("total pairs ", pairs.length);

// old gas junk
//const scaledL2GasPrice = await scaleL2GasPrice()

//const gasPriceToSend = scaledL2GasPrice
//console.log("gas price to send (scaled l2 gas)",gasPriceToSend)

  //const currentGasPrice = await PROVIDERS[CONFIG.CHAINNAME].getGasPrice();
 // let currentGasPriceGwei = ethers.utils.formatUnits(parseInt(scaledL2GasPrice), "gwei");

  // Convert the value to a number for easier comparisons
...
const maxOutResults = await Multicall(multiCallMaxOutArray);

// old gas junk
//console.log("lets send with gas price scaled ",gasPriceToSend)
  ...
  for (let z = 0; z < pairs.length; z++) {
   ...
   const vaultAddress = pairs[z].VAULT
    ...
    if (parseFloat(maxOut) === 0) {
      ...
      // try { amountIn = await contractWSigner.callStatic.computeExactAmountIn(maxOut);} catch (error) {
      //   console.log(error);
      // }
      ...
      try {
        ...
        percentageIncrements = Array.from({ length: 20 }, (_, i) => (i + 1) * 0.05);
...
for (let i = 0; i < results.length; i++) {
    ...
    const actualOutValue = ethers.utils.formatUnits(amountOutForPercentage, pairDecimals) * pairOutPrice;
    const actualInValue = ethers.utils.formatUnits(amountInOption, ADDRESS[CONFIG.CHAINNAME].PRIZETOKEN.DECIMALS) * prizeTokenPrice;

    // Calculate the $ profit for this swap
    const profit = actualOutValue - actualInValue;

    // Calculate the % profit for this swap
    const percentageProfit = (actualOutValue - actualInValue) / actualInValue;
    ...
    if (percentageProfit > bestPercentageProfit && amountInOption.lte(walletPrizeTokenBalance)) {
    ...
    tableData.push({
        "Max Out %": parseFloat((percentageIncrements[i] * 100).toFixed(2)),
...
const columns = ["Max Out %", "Amount Out", "Amount In", "Profit ($)", "% Profit"];

// Now, print the tableData
...
console.log(
    ...
    " and a profit of ",
    (ethers.utils.formatUnits(bestOptionOutPercentage, pairDecimals) * pairOutPrice - 
     ethers.utils.formatUnits(results[bestPercentageIndex], ADDRESS[CONFIG.CHAINNAME].PRIZETOKEN.DECIMALS) * prizeTokenPrice).toFixed(4)
);

// The condition now checks against both $ profit and % profit thresholds
if (bestProfit >= (profitThreshold - profitThreshold * 0.10) && bestPercentageProfit >= percentProfitThreshold && bestProfit > minimumGasCost) { 

          console.log("Optimizing further due to profit exceeding thresholds.");
      
          const basePercentage = percentageIncrements[mostProfitableIndex];
          percentageIncrements = Array.from({ length: 21 }, (_, i) => basePercentage + (i - 10) * 0.005)
...
for (let i = 0; i < results.length; i++) {
    ...
    // Calculate the profit for this swap
    const profit = 
        ethers.utils.formatUnits(amountOutForPercentage, pairDecimals) * pairOutPrice -
        ethers.utils.formatUnits(
            amountInOption,
            ADDRESS[CONFIG.CHAINNAME].PRIZETOKEN.DECIMALS
        ) * prizeTokenPrice;
    ...
    const actualOutValue = ethers.utils.formatUnits(amountOutForPercentage, pairDecimals) * pairOutPrice;
    const actualInValue = ethers.utils.formatUnits(
            amountInOption,
            ADDRESS[CONFIG.CHAINNAME].PRIZETOKEN.DECIMALS
        ) * prizeTokenPrice;
    
    const percentageProfit = ((actualOutValue - actualInValue) / actualInValue) * 100;
...
console.table(tableData,columns);
          ...
          bestOptionOut = maxOut
            .mul(Math.round(percentageIncrements[mostProfitableIndex] * 100))
            .div(100);
          bestOutValue = ethers.utils.formatUnits(bestOptionOut, pairDecimals) * pairOutPrice; 
          bestOptionIn = results[mostProfitableIndex];
          console.log(
...
let web3TotalGasCost, web3TotalGasCostUSD
      try {
        ...
        /*
        const gasEstimate = await CONTRACTS.LIQUIDATIONROUTERSIGNER[
          CONFIG.CHAINNAME
...
const poolFromAddress = "0xe0e7b7C5aE92Fe94D2ae677D81214D6Ad7A11C27"
const args = [
                  pairAddress,
...
//console.log("args",args)
        ...
        const data = CONTRACTS.LIQUIDATIONROUTERSIGNER[CONFIG.CHAINNAME].interface.encodeFunctionData(functionName, args);
        // calculate total gas cost in wei
        
         web3TotalGasCost  = await web3GasEstimate(data, CONFIG.CHAINID, poolFromAddress, ADDRESS[CONFIG.CHAINNAME].LIQUIDATIONROUTER);
        
        console.log("Gas Estimate through Web3 " + Number(web3TotalGasCost)/1e18 + " ETH")
         web3TotalGasCostUSD = Number(web3TotalGasCost).toFixed(2) * ethPrice / 1e18
...
const USDC = '0x7F5c764cBc14f9669B88837ca1490cCa17c31607'.toLowerCase()  // bridged
const WETH = '0x4200000000000000000000000000000000000006'
const PTUSDC = '0xE3B3a464ee575E8E25D2508918383b89c832f275'.toLowerCase() // bridged usdc vault #1
const POOL = ADDRESS[CONFIG.CHAINNAME].PRIZETOKEN.ADDRESS.toLowerCase()
...
const gasCouldBe = web3TotalGasCostUSD * .2
...
if(USESWAPPER && (pairOutAsset.toLowerCase() === PTUSDC || pairOutAsset.toLowerCase() === WETH || pairOutAsset.toLowerCase() === USDC || pairOutAsset.toLowerCase() === POOL) && outValue - (poolValue +gasCouldBe)> 0) {
...
const gasPercentageOfOutValue = (web3TotalGasCostUSD * 4.5) / bestOptionOutValue

console.log("gas percentage of out value",(gasPercentageOfOutValue*100).toFixed(2),"%")
...
let gasPercentageBigNumber = ethers.BigNumber.from(Math.round(gasPercentageOfOutValue * 10000).toString());

// Calculate wethOut first
let yieldForWeth = bestOptionOut.mul(gasPercentageBigNumber).div(PERCENT_10000);
...
console.log("swap params for 1inch swap back to pool",poolSwapParams)
...
console.log("swap params for 1inch to reimburse gas",gasSwapParams)
...
const newPoolCalldata = replaceAddressInCalldata(poolSwap.data,holderAddress, contractAddress);
console.log("pool data after address swap",newCalldata);

const newWethCalldata = replaceAddressInCalldata(wethSwap.data,holderAddress, contractAddress);
...
console.log("swap args",swapargs)
        // Encode the function call
        const swapdata = CONTRACTS.SWAPPERSIGNER[CONFIG.CHAINNAME].interface.encodeFunctionData(swapfunctionName, swapargs);
        // calculate total gas cost in wei
        
        const swapweb3TotalGasCost  = await web3GasEstimate(swapdata, CONFIG.CHAINID, ADDRESS[CONFIG.CHAINNAME].SWAPPER, ADDRESS[CONFIG.CHAINNAME].SWAPPER);
        
        console.log("SWAPPER Gas Estimate through Web3 " + Number(swapweb3TotalGasCost)/1e18 + " ETH")
        swapweb3TotalGasCostUSD = Number(swapweb3TotalGasCost).toFixed(2) * ethPrice / 1e18
...
return
const sendIt = await CONTRACTS.SWAPPERSIGNER[CONFIG.CHAINNAME].processSwaps(pairAddress,
                  bestOptionOut,
                  maxToSendWithSlippage,
                  poolSwap.data,
                  wethSwap.data,          { maxPriorityFeePerGas: "1000011",    gasLimit: "1700000" })
...
/*
        const gasEstimateInteger = parseInt(gasEstimate);
        const gasEstimateWithBuffer = parseInt(
          gasEstimateInteger + gasEstimateInteger * 0.2
        );
*/
        //  Calculate total gas cost in gwei
        
// lets try scaled version
// console.log("gas price to send",gasPriceToSend," sending with slippage of ",slippage," bps")

// let totalGasCostGwei = (gasPriceToSend/1e9)  * gasEstimateWithBuffer

//let totalGasCostGwei = adjustedGasPriceGwei * gasEstimateWithBuffer;

//let totalGasCostGwei = adjustedGasPriceGwei * gasEstimateWithBuffer;
     //   totalGasCostGwei = totalGasCostGwei * gasCostFactor;
        //  Convert total gas cost to ether

   //     const totalGasCostEther = totalGasCostGwei / 1e9;

        //  Convert total gas cost in ether to USD
     /*   const totalGasCostUSD = totalGasCostEther * ethPrice;
        if ((totalGasCostGwei/1e9) > maxGas) {
          console.log("gas estimate too high ", gasEstimateWithBuffer);
          continue;
        }
*/

        // we need real a better gas cost here but yea
        const profit = outValue - poolValue - web3TotalGasCostUSD;
        const totalCost = poolValue + web3TotalGasCostUSD;


/*try {
const gasCostsTx = await CONTRACTS.LIQUIDATIONROUTERSIGNER[
          CONFIG.CHAINNAME
        ].populateTransaction.swapExactAmountOut(
          pairAddress,
          CONFIG.WALLET,
          bestOptionOut,
          maxToSendWithSlippage,
          unixTimestamp,
          { maxPriorityFeePerGas: "1000000", gasLimit: gasEstimateWithBuffer.toString() }
        );
/*const gasCosts = await estimateGas(gasCostsTx)

console.log("ethers gas costs ",gasCosts)
gasCostsUsd = ethers.utils.formatUnits(gasCosts.gas,9) * ethPrice
console.log("ethers gas cost in usd ",gasCostsUsd) 
}
catch(e){console.log("error estiamting gas cost",e)}
*/  
      ...
      console.log(
        ...
        if (profit < profitThreshold || (totalCost * profitPercentage) > outValue) {
          console.log("not meeting profit threshold of $", profitThreshold," AND > ",(profitPercentage*100).toFixed(0),"%");
          continue;
        }
...
walletPrizeTokenBalance = walletPrizeTokenBalance.sub(maxToSendWithSlippage)
...
const L2transactionCost = Number(txReceipt.gasUsed * txReceipt.effectiveGasPrice) / 1e18;

  console.log("L2 Gas fees (in ETH) " + L2transactionCost)

  const alchemyReceipt = await AlchemyTransactionReceipt(txReceipt.transactionHash)
  ...
  console.log("Total Gas fees (in ETH) " + totalTransactionCost)

  totalTransasactionCostDollar = totalTransactionCost * ethPrice

  console.log("Total Gas fees $ " + totalTransasactionCostDollar)
        ...
        const logs = GetLogs(txReceipt, ABI.SWAPEVENTS);
	...
	let poolSpent, amountReceived
		...
		totalPoolSpent += poolSpent;
            assetsReceived.push({
              asset: pairOutSymbol,
              amount: ethers.utils.formatUnits(args.amountOut, pairDecimals),
            });
...
// Initialization
let fileData = [];
console.log("initial file write process")
if (fs.existsSync(dataFilePath)) {
    try {
        fileData = JSON.parse(fs.readFileSync(dataFilePath, 'utf-8'));
        console.log('Initial data read from file:', JSON.stringify(fileData, null, 2));
    } catch (error) {
        console.error('Error reading or parsing file data:', error);
    }
} else {
    console.log(`File at path ${dataFilePath} does not exist. Starting with empty data.`);
}
...
console.log('Data after pushing new entry:', JSON.stringify(fileData, null, 2));

try {
    fs.writeFileSync(dataFilePath, JSON.stringify(fileData, null, 2), 'utf-8');
    console.log(`Data successfully written to file at ${dataFilePath}.`);
} catch (error) {
  ...
  console.log(assetsReceived);}else{console.log(section("No liquidations completed (this is buggy)"))}
} else {console.log("liquidations are off while draw is being awarded")}
}
