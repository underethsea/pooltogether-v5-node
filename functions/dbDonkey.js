...
async function AddClaim(network, claim) {
    try {
        ...
        const addClaimQuery = `
            INSERT INTO claims (network, block, hash, draw, vault, winner, payout, miner, fee, tier, index) 
     ...
     console.log("claim added to db, tx ",claim.hash," winner ",claim.winner," tier/index ",claim.tier,"/",claim.index)
    } catch (error) {
        // Handle error
